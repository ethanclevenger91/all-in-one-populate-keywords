...
class Ai1_Keywords_Populate {
				...
				echo '<div class="keywords-wrapper">';
					...
					foreach($keys as $key) {
						echo '<div class="pop-key-input-wrap">';
							...
							echo '<button type="button" class="remove-this-key">Remove This Key</button>';
						echo '</div>';
					}
				echo '</div>';
				echo '<div class="bottom-button-wrap">';
					echo '<button type="button" onclick="addInput()">Add Another Keyword</button>';
					echo '<button type="button" onclick="removeAllKeys()">Remove All Keywords</button>';
					echo '<input class="ai1-keywords-submit" type="submit" value="Populate Keywords">';
				echo '</div>';
			echo '</form>';
		echo '</div>';
	}
	...
	function enqueue_admin_scripts($hook) {
		if('tools_page_ai1_populate_keywords' != $hook) return;
